## ES6 문법 문제
```javascript=
1. 다음의 코드를 es6문법을 이용하여 재작성하시오.
let name="song's fruit store"
...
function findStore(obj){
    let {name, address:{city}} = obj // 추가
    return name="song store" && city == "Seoul"
}
...
function getNumber(){
    ...
    // 나의 답
    let [first, second, third, forth, fiveth, sixth] = array
    return {first, third, forth,...rest}
    // 정답
